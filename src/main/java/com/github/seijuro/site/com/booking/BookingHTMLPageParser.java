...
@Log4j2
public class BookingHTMLPageParser implements HTMLPageParser {
    /**
    ...
    public HotelData parseHotel(SearchResultType type, Element hotel) {
        HotelData.Builder hotelBuilder = new HotelData.Builder();
        ...
        if (type == SearchResultType.NORMAL) {
            ...
            hotelBuilder.setPrice(priceElements.size() > 0 ? priceElements.get(0).text() : StringUtils.EMPTY);
            if (freeCanellationElements.size() > 0) { hotelBuilder.addCondition(freeCanellationElements.get(0).text()); }
        }
    ...
    public void parse(String html) {
        ...
        for (Element hotel : hotels) {
            ...
            try {
                writer.write(parseHotel(type, hotel));
            }
